...
exports.lookup = function(db){
	return function(req, res){
		...
		if(start_port && end_port){
			...
			ports_collection.findOne({"info.search_term": new RegExp(start_port, 'i')}, function(err, docs){
				if(docs != null){
					...
					if(docs != null){
							...
							var and = [{"voyages.ports.port": start_port}, {"voyages.ports.port": end_port}];
							query["$and"] = and;
							var options = {"sort":"voyages.ports.port.eta"};
							schedules.find(query).toArray(function(err, docs){

								var response_data = [];
								// loop through all the returned schedules , arrange data in order of eta
								for(var doc in docs){
									var voyages = docs[doc]['voyages'];
									for( var voyage in voyages){
										var ports = voyages[voyage]['ports'];
										...
										*/
										for (var pindex in ports){
											if(ports[pindex]['port'] == start_port){
												start_port_found = true;
												start_port_etd = ports[pindex]['etd'];
											}else if(ports[pindex]['port'] == end_port){
			...
			console.log("Query", query, "Projection", projection);
